...
foreach($x in $outer){
   ...
   function Find-ADUser {
        ...
        Get-ADUser -Filter "givenname -like '$ADUserFirstName'" -Properties "Description","OfficePhone","telephoneNumber"
# User has option to enter their own filter option with the associated info if they require more searching
        ...
        if (($UserSearchFilterPrompt) -eq "yes" -or ($UserSearchFilterPrompt) -eq "y") {
            ...
            Get-ADUser -Filter {$ADUserFilter -like $ADUserFilterInfo} -Properties "Description","OfficePhone","telephoneNumber"
